...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \begin{tabular}{c c}
             Cheramy Benoît & Moczygeba Joris \\
...
\subsection{État des Lieux (Audit et Benchmark)}
Avant de débuter le développement, une phase d'analyse critique a été menée :
...
Face à l'obsolescence de l'outil actuel, nous avons obtenu l'accord du client pour \textbf{partir d'une page vierge} afin de garantir une architecture propre, évolutive et conforme aux attentes des utilisateurs.
...
\begin{itemize}[label=\textbullet, itemsep=5pt]
    ...
    \item \textbf{La cartographie des entreprises :} À la suite de nos échanges avec le client, nous avons intégré une dimension géographique au projet. L'outil propose une carte des entreprises représentées au sein du réseau. Cette interface permet de visualiser en un coup d'œil l'influence des Ensicaennais sur le territoire et d'identifier les bassins d'emploi où la communauté est la plus présente.
    \item \textbf{Le volet "Carrière" :} À notre initiative, nous avons intégré un module de dépôt d'offres de stages et d'emplois. Cette fonctionnalité crée un cercle vertueux d'entraide, facilitant directement l'employabilité des Ensicaennais et renforçant l'utilité concrète du réseau au quotidien.
    \item \textbf{Gestion de l'actualité et des événements :} Un espace dédié permet à l'association (notamment à la secrétaire, Mme Catherine Conte-Marion) d'administrer du contenu de manière autonome. Cet outil mettra en lumière l'engagement des anciens étudiants et diffusera les événements clés, garantissant un réseau vivant et ouvert à toute proposition d’évènement.
\end{itemize}
...
\begin{itemize}[label=\textbullet, itemsep=5pt]
    ...
    \item \textbf{Le module "Carrière"} permettant le dépôt et la consultation d'offres de stages et d'emplois, afin de transformer la plateforme en un véritable hub d’embauche pour les Ensiceannais.
    \item \textbf{Une interface d'administration simplifiée} a été mise à la disposition de l'association permettant à la secrétaire de gérer les actualités et les événements en totale autonomie, sans intervention technique.
\end{itemize}
...
L'obstacle le plus significatif a été l'impossibilité d'accéder aux bases de données initialement promises, suite à des directives de sécurité nationale (DGSI) restreignant l'accès aux infrastructures sensibles.
...
\begin{itemize}[label=\textbullet, itemsep=5pt]
    \item \textbf{Gérer les attentes :} Expliquer de manière pédagogique les limitations techniques imposées par les contraintes de sécurité de la DGSI.
...
\begin{itemize}[label=\textbullet, itemsep=5pt]
    ...
    \item \textbf{Architecture sécurisée :} La sécurité a été placée au centre du développement, notamment via l'intégration de la connexion Microsoft Auth et la gestion de sessions sécurisées (tokens JWT), garantissant ainsi la protection des données des membres.
\end{itemize}
...
\begin{itemize}[label=\textbullet, itemsep=5pt]
    ...
    \begin{itemize}
        ...
        \item Versioning de code via Git.
    \end{itemize}
