...
class _CompaniesDirectoryPageState extends State<CompaniesDirectoryPage> {
  ...
  List<Map<String, dynamic>> _getMapData() {
    // Si rien n'est sélectionné, on affiche toutes les entreprises sur la carte
    if (_selectedItem == null) {
      return viewModel.allCompanies; 
    }

    // Si on a sélectionné une Entreprise précise
    if (_groupBy == 'Entreprise') {
      return viewModel.allCompanies.where((c) => 
        c['nom_entreprise'] == _selectedItem!['nom_entreprise'] && 
        c['ville'] == _selectedItem!['ville'] // Sécurité pour les homonymes
      ).toList();
    } 
    // Si on a sélectionné une Ville (on affiche toutes les entreprises de cette ville)
    else {
      return viewModel.allCompanies.where((c) => 
